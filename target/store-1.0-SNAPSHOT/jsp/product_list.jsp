		...
		<div style="width:380px;margin:0 auto;margin-top:50px;">
			<ul class="pagination" style="text-align:center; margin-top:10px;">
                ...
                <c:if test="${bean.curPage gt 1}">
                    <li><a
                            href='${pageContext.request.contextPath}/product?method=findProductByCid&cid=${param.cid}&thisPage=${bean.curPage-1}' aria-label="Previous"><span aria-hidden="true">&laquo;</span></a></li>
                </c:if>
				...
				<c:forEach begin="1" end="${bean.totalPage}" var="i">
                    ...
                    <c:if test="${i eq bean.curPage}">
                        <li class="active">
                            <a
                                    href="${pageContext.request.contextPath}/product?method=findProductByCid&cid=${param.cid}&thisPage=${i}" >${i}</a>
                        </li>
                    </c:if>
                ...
                <c:if test="${bean.curPage ne bean.totalPage}">
                    <li>
                        <a
                                href="${pageContext.request.contextPath}/product?method=findProductByCid&cid=${param.cid}&thisPage=${bean.curPage+1}" aria-label="Next">
                            <span aria-hidden="true">&raquo;</span>
                        </a>
                    </li>
                </c:if>
		...
		<div style="width:1210px;margin:0 auto; padding: 0 9px;border: 1px solid #ddd;border-top: 2px solid #999;height: 246px;">
			...
			<div style="overflow: hidden;">
				<ul style="list-style: none;">
					<%--遍历历史记录--%>
					<c:choose>
						<c:when test="${cookie.record == null}">
							<c:out value="你现在还没有访问任何商品..."/>
						</c:when>
						<c:otherwise>
							<c:forEach items='${fn:split(cookie.record.value, "-")}'
									   var="id">

								<li style="width: 150px;height: 216px;float: left;margin: 0 8px 0
								0;padding: 0
								18px 15px;text-align: center;"><img
										src='${pageContext.request.contextPath}/${id}'
										width="80px"
										height="90px" /></li>
							</c:forEach>
						</c:otherwise>
